...
public class AiController {
    ...
    @MessageMapping("/ai")
    public void sendAi(@RequestBody InteractionDto interactionDto,
        ...
        if (interactionDto.getButton() == 3) {
            AiReqDto aiReqDto = AiReqDto.builder()
                ...
                .build();
            AiResDto aiResDto;
            log.info("[sendAi] : 메세시 수신 성공, message : {}, link : {}", interactionDto.getMessage(),
                interactionDto.getLink());
            ...
            } else if (INCREASE_KEY.equals(aiReqDto.getMsg())) {
                aiResDto = AiResDto.builder().prediction(5).build();
                log.info("[sendAi] : 보이스 피싱 감지 수신, prediction : {}", aiResDto.getPrediction());
            } else {
            ...
            log.info("[sendAi] : 버튼 수신 성공, button : {}, userType : {}", interactionDto.getButton(),
                interactionDto.getUserType());
